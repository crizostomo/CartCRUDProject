...
public interface CartRepository extends JpaRepository<Cart, Long> {
    ...
    @Query("select u from Cart u where (:q is null or UPPER(u.name) like %:q% or UPPER(u.description) like %:q%) " +
            "and (:min_price is null or u.price >= :min_price) and (:max_price is null or u.price <= :max_price)")
